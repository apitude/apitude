...
class Worker extends BaseCommand
{
    ...
    public function configure()
    {
        ...
        $this->setName('kue:work')
            ...
            ->addOption('qname', 'q', InputOption::VALUE_REQUIRED, 'Queue name to process', 'kue:default')
    ...
    protected function setupWorker(\Kue\Worker $worker)
    {
        $log = $this->log;

        $worker->on('init', function(Job $job) use ($log) {
            $log->info(sprintf('Accepted job %s', get_class($job)), array('job' => $job));
        });
        ...
        $worker->on('exception', function(Job $job, \Exception $exception) use ($log) {
            $log->error(sprintf('Job "%s" failed: %s', get_class($job), $exception), array('job' => $job));
        });

        $worker->on('success', function(Job $job) use ($log) {
            $log->info(sprintf('Job "%s" finished successfully', get_class($job)), array('job' => $job));
        });
